...
def fourier_transform(signal, frequencies, sampled_times):
    ...
    for i in range(num_freqs):
        ft_result_real[i] = np.trapz(signal * np.cos(2*np.pi*frequencies[i]*sampled_times),sampled_times)
        ft_result_imag[i] = -np.trapz(signal * np.sin(2*np.pi*frequencies[i]*sampled_times),sampled_times)
...
wavfile.write('denoised_audio2.wav', sample_rate, filtered_data)
